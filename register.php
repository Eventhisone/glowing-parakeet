...
// This script performs an INSERT query to add a record to a table
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['last_name'])) {
        trim($errors[] = 'You forgot to enter your last name.');
    } else {
	...
	// Make sure the email hasn't been used already
	$eq = "SELECT user_id FROM users WHERE email='$e'";
	$er = @mysqli_query($dbc, $eq);
	...
	if (mysqli_num_rows($er) == 0) {
            ...
            if ($r) {
                ...
                <p>You are now successfully registered. In Chapter 12 you will actually be able to log in!
                </p><br></b>';
	    ...
	    echo "<p class='error'>The email <em>$e</em> has already been used.</p>";	
	}
