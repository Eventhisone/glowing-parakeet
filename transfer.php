...
$dbc = mysqli_connect('localhost', 'root', 'SillyRabbit', 'banking') OR die('Could not connect to 
    ...
    is_numeric($_POST['from']) && is_numeric($_POST['to']) && is_numeric($_POST['amount']) ) {
        ...
        if ($amount > $row['balance']) {
            ...
            if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
                ...
                mysqli_rollback($dbc);
                echo '<p>The transfer could not be made due to a system error. We 
        ...
        echo '<p>Please select a valid "from" and "to" account and enter a numeric 
...
// Get all the accounts as OPTIONa for the SELECT menus:
